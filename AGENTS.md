...
## Current Defense MVP local CPU engineering and publication authorization

1. On 2026-09-01, the user explicitly authorized Codex to design and implement a separate local CPU-only defense MVP titled provisionally "基于约束式多目标排序的指令视频编辑候选选择与盲评系统", with a target delivery date of 2026-09-08.
2. The authorization covers local code, tests, configuration, documentation, `DEVLOG.md`, CPU-only analysis of user-provided audited E0 media, report generation, slide preparation, demo recording support, audited commits, and ordinary pushes to `main` without force-pushing.
3. The Defense MVP must remain logically and structurally separate from the fixed E1/E2 research protocols. It may reuse tested utilities but must not weaken, bypass, or relabel the E1 `PASS_PROVISIONAL` / `reward-v0.yaml` gate or the formal E2 provenance requirements.
4. The local agent must not connect to or operate the school server, read or write DATA4, launch GPU work, load a real judge or video generation model, modify sealed E0 outputs, or create remote experiment directories. Server-side packaging is performed by the user or a separately authorized server-side agent using read-only source checks and a new package destination.
5. The Defense MVP may report CPU metric measurements and formal human annotations obtained under its own documented protocol. Mock, synthetic, replay, or incomplete inputs must remain explicitly non-research and must never be presented as real experimental evidence.
6. The project completion criterion is a reproducible system, complete three-method comparison, valid two-annotator blind evaluation, honest statistical report, and defense artifacts. A positive win-rate result is not required, and negative or inconclusive findings must be preserved rather than tuned away.
