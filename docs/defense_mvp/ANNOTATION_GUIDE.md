...
工程施工中的 64 条测试回答全是 practice 固定值；2026-09-04 两位真人正式标注已另行完成。
当前封存状态见 D3_FORMAL_ANNOTATION_RECEIPT；以下保留标注时使用的操作协议。
...
页面显示每路读取进度和“正在解码 / 可播放 / 失败”。等三路都可播放后，
“同步播放”才可点击。读取超时会自动重试一次；仍失败时点击“重新加载视频”，
当前题目、方向和已填写草稿都保留，加载完成后需重新播放三路。
会话失效时使用本次服务入口恢复；解码错误重试后仍存在则联系维护者，不填答案代替故障。
...
为解决嵌入浏览器原生媒体读取停滞，页面经同源认证完整读取本题三路媒体，再用临时
blob URL 播放；字节保持一致，没有再次转码。切题/重载会取消旧请求并释放旧视频，
不预读后续题目。网络读取单次20秒、最多自动重试一次，解码另有20秒超时；
这些是故障恢复限制，不是评审作答时间限制。Node只用于开发测试，用户启动服务不需要Node。
