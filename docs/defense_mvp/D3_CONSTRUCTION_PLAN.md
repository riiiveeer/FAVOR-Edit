...
## D3 加载修复（2026-09-03 用户试用反馈）

现场为 practice 第1题，API正常但三路长期无首帧，原生媒体请求未形成可用播放。
终端出现一次匿名403，不能据此断言唯一底层原因。现有页面还缺少超时、进度与局部恢复。

1. 用与题目API相同的同源认证 fetch 读取当前三路媒体，显示字节进度，20秒超时，
   网络故障自动再试一次；完整接收后创建临时 blob URL，避免嵌入浏览器原生流请求停滞。
2. 三路保留原有无损WebM字节；仍走当前view/角色路由、SHA校验和服务端提交门禁。
   加载器只保留当前题，切题/重试取消旧请求并回收URL；限制单路64MiB内存。
3. 每路显示读取/解码/可播放/错误；重新加载不换题、不改方向、不清表单草稿。
   旧题事件/过期请求不能污染当前状态；媒体就绪请求串行化，避免晚到的false覆盖true。
4. 用Node微型媒体/网络替身验证中断、超时、HTTP错误、长度错误、重试和资源回收；
